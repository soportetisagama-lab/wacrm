...
export interface Contact {
  ...
  account_id: string;
  /**
   * Nullable since migration 042: a contact created from a username-only
   * inbound message (no phone in the webhook payload — see BSUID
   * below) has no phone on file. Every contact created any other way
   * (manual form, CSV import, public API) still always has one.
   */
  phone: string | null;
  ...
  /**
   * Business-scoped user id (migration 042) — set when this contact
   * was first seen via a username-only inbound message (no phone on
   * the payload). Absent for every contact created the normal way.
   */
  whatsapp_user_id?: string | null;
  /** The contact's WhatsApp @username, if Meta reported one. Display
   *  only — whatsapp_user_id (BSUID) is the stable identifier. */
...
export interface WhatsAppConfig {
  ...
  /**
   * Migration 042. Gates ONLY whether the REQUEST_CONTACT_INFO button
   * is proactively sent to a BSUID-only contact (one with no phone on
   * file) on their first such inbound message. Does NOT gate whether
   * that contact/conversation gets created — that always happens,
   * independent of this flag. Defaults to false.
   */
  bsuid_request_contact_info_enabled: boolean;
}
